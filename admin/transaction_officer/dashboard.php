...
try {
    ...
    $stmtDep = $pdo->query("SELECT COALESCE(SUM(amount), 0.00) FROM transactions WHERE type IN ('deposit', 'adjustment_credit') AND status IN ('completed', 'success', 'successful')");
    $total_deposits = (float)$stmtDep->fetchColumn();

    // Total Successful Withdrawals
    $stmtWth = $pdo->query("SELECT COALESCE(SUM(amount), 0.00) FROM transactions WHERE type IN ('withdraw', 'payout', 'adjustment_debit') AND status IN ('completed', 'success', 'successful')");
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        ...
        .btn-action { padding: 8px 16px; border-radius: 6px; background: #002b49; color: #fff; text-decoration: none; font-size: 13px; font-weight: 600; display: inline-flex; align-items: center; gap: 8px; border: none; cursor: pointer; white-space: nowrap; transition: 0.3s; }
...
<div class="admin-container">
    <div class="sidebar" id="transSidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="escalations.php"><i class="fas fa-exclamation-circle"></i> Escalated Issues</a></li>
                    ...
                    <tbody>
                        <?php if (!empty($recentTxns)): ?>
                            <?php foreach ($recentTxns as $t): ?>
                                <tr>
                                    <td><strong><?= htmlspecialchars($t['transaction_ref'] ?? ($t['reference_no'] ?? 'N/A')) ?></strong></td>
                                    <td><?= htmlspecialchars($t['full_name']) ?><br><small style="color:#777;"><?= htmlspecialchars($t['phone_number']) ?></small></td>
                                    <td><span style="text-transform: uppercase; font-weight: bold; font-size: 12px;"><?= htmlspecialchars(str_replace('_', ' ', $t['type'])) ?></span></td>
                                    <td style="font-weight: bold; color: <?= in_array(strtolower($t['type']), ['deposit', 'credit', 'adjustment_credit']) ? '#27ae60' : '#c0392b' ?>;">
                                        <?= in_array(strtolower($t['type']), ['deposit', 'credit', 'adjustment_credit']) ? '+' : '-' ?> TZS <?= number_format($t['amount'], 2) ?>
                                    </td>
                                    <td>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="6" style="text-align: center; color: #777; padding: 20px;">No transaction ledger entries found.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
